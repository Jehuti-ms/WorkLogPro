...
// ===========================
// UTILITY FUNCTIONS (Defined first to be available everywhere)
// ===========================
...
// ===========================
// SAFE NOTIFICATION WRAPPER (Used before NotificationSystem is fully defined)
// ===========================

function safeNotify(type, message) {
  // Try to use NotificationSystem if available
  if (window.NotificationSystem && window.NotificationSystem[`notify${type}`]) {
    window.NotificationSystem[`notify${type}`](message);
  } else {
...
const FormAutoClear = {
  ...
  handleSuccess(formId, customData = {}) {
    ...
    if (config.successMessage) {
      safeNotify('Success', config.successMessage);
    }
  }
};
...
const SyncBar = {
    ...
    if (autoSyncCheckbox) {
      ...
      autoSyncCheckbox.addEventListener('change', (e) => {
        ...
        if (isAutoSyncEnabled) {
          ...
          this.startAutoSync();
          safeNotify('Success', 'Auto-sync enabled - syncing every 60 seconds');
        } else {
          autoSyncText.textContent = 'Manual';
          if (syncIndicator) {
            syncIndicator.style.backgroundColor = '#ef4444';
          ...
          this.stopAutoSync();
          safeNotify('Info', 'Auto-sync disabled');
        }
      });
    }
  },
  ...
  setupExportCloudButton() {
    if (exportCloudBtn) {
      exportCloudBtn.addEventListener('click', async () => {
        safeNotify('Info', 'Export to cloud feature coming soon');
      });
    }
  },
  ...
  setupImportCloudButton() {
    if (importCloudBtn) {
      importCloudBtn.addEventListener('click', async () => {
        safeNotify('Info', 'Import from cloud feature coming soon');
      });
    }
  },
    ...
    if (syncStatsBtn) {
      syncStatsBtn.addEventListener('click', async () => {
        ...
        if (user) {
          ...
          safeNotify('Success', 'Stats recalculated and synced');
        }
      });
    }
  },
  ...
  setupExportDataButton() {
    if (exportDataBtn) {
      exportDataBtn.addEventListener('click', () => {
        safeNotify('Info', 'Export data feature coming soon');
      });
    }
  },
  ...
  setupImportDataButton() {
    if (importDataBtn) {
      importDataBtn.addEventListener('click', () => {
        safeNotify('Info', 'Import data feature coming soon');
      });
    }
  },
    ...
    if (clearDataBtn) {
      clearDataBtn.addEventListener('click', () => {
        if (confirm('Are you sure you want to clear all local data? This cannot be undone.')) {
          ...
          safeNotify('Success', 'All local data cleared');
  ...
  async performSync(mode = 'manual') {
    const user = auth.currentUser;
    if (!user) {
      safeNotify('Error', 'Please log in to sync');
      return;
    }
    ...
    try {
      ...
      safeNotify('Success', `${mode === 'auto' ? 'Auto-' : ''}Sync completed successfully`);
      ...
      safeNotify('Error', `Sync failed: ${error.message}`);
    }
  }
};
...
// ===========================
// FORM CALCULATION FUNCTIONS (Need to be defined before setupFormHandlers)
// ===========================
...
// ===========================
// STUDENT FORM FUNCTIONS (Fixed to use safeNotify)
// ===========================
...
async function handleStudentSubmit(e) {
    ...
    try {
        ...
        safeNotify('Error', 'Failed to add student');
    }
}

async function editStudent(id) {
  safeNotify('Info', `Edit student ${id} - Feature coming soon`);
}

async function deleteStudent(id) {
  if (confirm('Are you sure you want to delete this student?')) {
    safeNotify('Info', `Delete student ${id} - Feature coming soon`);
  }
}
...
async function startEditHours(id) {
  try {
    ...
    if (!entry) {
      safeNotify('Error', 'Hours entry not found');
      return;
    }
    ...
    if (!submitBtn) {
      ...
      safeNotify('Error', 'Cannot enter edit mode - form not found');
      return;
    }
    ...
    safeNotify('Info', 'Edit mode activated. Update the form and click "Update Hours"');
    
  } catch (error) {
    console.error('Error starting edit:', error);
    safeNotify('Error', 'Failed to start editing');
  }
}
...
function cancelEditHours() {
  ...
  safeNotify('Info', 'Edit cancelled');
}
...
async function deleteHours(id) {
  ...
  try {
    ...
    safeNotify('Success', 'Hours entry deleted successfully');
    
  } catch (error) {
    console.error('Error deleting hours:', error);
    safeNotify('Error', 'Failed to delete hours entry');
  }
}
  ...
  try {
    if (currentEditHoursId) {
      ...
      if (entryIndex !== -1) {
        ...
        safeNotify('Success', 'Hours updated successfully');
        currentEditHoursId = null;
    ...
    safeNotify('Error', 'Failed to save hours');
  }
}

// ===========================
// MARKS TAB FUNCTIONS (Fixed to use safeNotify)
// ===========================
...
async function startEditMark(id) {
  try {
    ...
    if (!entry) {
      safeNotify('Error', 'Mark entry not found');
      return;
    }
    ...
    safeNotify('Info', 'Edit mode activated. Update the form and click "Update Mark"');
...
async function deleteMark(id) {
  ...
  try {
    ...
    safeNotify('Success', 'Mark deleted successfully');
    
  } catch (error) {
    console.error('Error deleting mark:', error);
    safeNotify('Error', 'Failed to delete mark');
  }
}
  ...
  try {
    if (currentEditMarksId) {
      ...
      if (entryIndex !== -1) {
        ...
        safeNotify('Success', 'Mark updated successfully');
        currentEditMarksId = null;
    ...
    safeNotify('Error', 'Failed to save mark');
  }
}

// ===========================
// ATTENDANCE TAB FUNCTIONS (Fixed to use safeNotify)
// ===========================
...
async function startEditAttendance(id) {
  try {
    ...
    if (!entry) {
      safeNotify('Error', 'Attendance record not found');
      return;
    }
    ...
    safeNotify('Info', 'Edit mode activated. Update the form and click "Update Attendance"');
...
async function deleteAttendance(id) {
  ...
  try {
    ...
    safeNotify('Success', 'Attendance record deleted successfully');
    
  } catch (error) {
    console.error('Error deleting attendance:', error);
    safeNotify('Error', 'Failed to delete attendance record');
  }
}
  ...
  try {
    if (currentEditAttendanceId) {
      ...
      if (entryIndex !== -1) {
        ...
        safeNotify('Success', 'Attendance updated successfully');
        currentEditAttendanceId = null;
    ...
    safeNotify('Error', 'Failed to save attendance');
  }
}

// ===========================
// PAYMENT MANAGEMENT FUNCTIONS (Fixed to use safeNotify)
// ===========================
...
async function startEditPayment(id) {
  try {
    ...
    if (!entry) {
      safeNotify('Error', 'Payment not found');
      return;
    }
    ...
    safeNotify('Info', 'Edit mode activated. Update the form and click "Update Payment"');
...
async function deletePayment(id) {
  ...
  try {
    ...
    safeNotify('Success', 'Payment deleted successfully');
    
  } catch (error) {
    console.error('Error deleting payment:', error);
    safeNotify('Error', 'Failed to delete payment');
  }
}
...
function quickAddPayment(studentName) {
  ...
  safeNotify('Info', `Quick payment mode for ${studentName}`);
}
  ...
  try {
    if (currentEditPaymentId) {
      ...
      if (entryIndex !== -1) {
        ...
        safeNotify('Success', 'Payment updated successfully');
        currentEditPaymentId = null;
    ...
    safeNotify('Error', 'Failed to save payment');
  }
}
...
function selectAllStudents() {
  ...
  if (!attendanceContainer) {
    ...
    safeNotify('Error', 'Attendance form not loaded properly');
    return;
  }
  ...
  if (checkboxes.length === 0) {
    ...
    safeNotify('Warning', 'No students available. Please add students first.');
    return;
  }
...
function clearAttendanceForm() {
  ...
  if (form) {
    ...
    safeNotify('Info', 'Attendance form cleared');
  }
}
...
function clearStudentForm() {
  ...
  if (form) {
    ...
    safeNotify('Info', 'Student form cleared');
  }
}
...
function resetHoursForm() {
  ...
  if (form) {
    ...
    calculateTotalPay();
    safeNotify('Info', 'Hours form cleared');
  }
}
...
async function saveDefaultRate() {
  const user = auth.currentUser;
  if (!user) {
    safeNotify('Error', 'Please log in to save default rate');
    return;
  }
  
  const defaultBaseRateInput = document.getElementById('defaultBaseRate');
  if (!defaultBaseRateInput) {
    safeNotify('Error', 'Default rate input not found');
    return;
  }
  
  const newRate = safeNumber(defaultBaseRateInput.value);
  if (newRate <= 0) {
    safeNotify('Error', 'Please enter a valid rate greater than 0');
    return;
  }
  ...
  try {
    ...
    if (success) {
      safeNotify('Success', `Default rate saved: $${fmtMoney(newRate)}/session`);
      initializeDefaultRate(newRate);
    }
  } catch (error) {
    console.error('Error saving default rate:', error);
    safeNotify('Error', 'Failed to save default rate');
  }
}
...
async function applyDefaultRateToAll() {
  const user = auth.currentUser;
  if (!user) {
    safeNotify('Error', 'Please log in to apply default rate');
    return;
  }
  ...
  if (confirm('Apply the default rate to ALL existing students? This cannot be undone.')) {
    ...
    if (newRate > 0) {
      ...
      safeNotify('Error', 'Please set a valid default rate first');
    }
  }
}
...
function useDefaultRate() {
  ...
  if (studentRateInput && defaultBaseRateInput) {
    ...
    safeNotify('Info', 'Default rate applied to student form');
  }
}
...
function useDefaultRateInHours() {
  ...
  if (baseRateInput && defaultBaseRateInput) {
    ...
    safeNotify('Info', 'Default rate applied to hours form');
    calculateTotalPay();
  }
}
...
async function generateWeeklyReport(startDate, endDate) {
  try {
    ...
    safeNotify('Error', 'Failed to generate weekly report');
  }
}
...
async function generateBiWeeklyReport(startDate, endDate) {
  try {
    ...
    safeNotify('Error', 'Failed to generate bi-weekly report');
  }
}
...
async function generateMonthlyReport(startDate, endDate) {
  try {
    ...
    safeNotify('Error', 'Failed to generate monthly report');
  }
}
...
async function generateSubjectReport(startDate, endDate) {
  try {
    ...
    safeNotify('Error', 'Failed to generate subject report');
  }
}
...
async function applyDefaultRateToAllStudents(uid, newRate) {
  try {
    ...
    if (updateCount > 0) {
      ...
      safeNotify('Success', `Applied default rate to ${updateCount} students`);
      console.log(`✅ Applied default rate to ${updateCount} students`);
    } else {
      safeNotify('Info', 'No students found to update');
    }
    
    return updateCount;
  } catch (error) {
    console.error('Error applying default rate to all students:', error);
    safeNotify('Error', 'Failed to apply default rate to all students');
    return 0;
  }
}
    ...
    logoutBtn.addEventListener('click', async () => {
      if (confirm('Are you sure you want to logout?')) {
        try {
          ...
          safeNotify('Error', 'Logout failed');
        }
      }
    });
  }
...
// Export NotificationSystem first

// reports.js - Complete reports functionality
...
class ReportManager {
    ...
    displayReport(type) {
        ...
        switch(type) {
            case 'weekly':
                report = this.dataManager.generateWeeklyReport();
                title = 'Weekly Report';
                break;
            case 'biweekly':
                report = this.dataManager.generateBiWeeklyReport();
                title = 'Bi-Weekly Report';
                break;
            case 'monthly':
                report = this.dataManager.generateMonthlyReport();
    ...
    showSubjectSelector() {
        ...
        document.getElementById('generate-subject-report').addEventListener('click', () => {
            const select = document.getElementById('subject-select');
            const custom = document.getElementById('custom-subject').value.trim();
            
            let subject = select.value;
            if (custom) {
                subject = custom.toLowerCase();
            }
            ...
            const report = this.dataManager.generateSubjectReport(subject);
    ...
    showPDFOptions() {
        ...
        html += '<input type="radio" id="pdf-weekly" name="pdf-type" value="weekly" checked>';
        html += '<label for="pdf-weekly" style="margin-left: 8px;">Weekly Report</label>';
        html += '</div>';
        
        html += '<div class="pdf-option" style="margin: 10px 0;">';
        html += '<input type="radio" id="pdf-biweekly" name="pdf-type" value="biweekly">';
        html += '<label for="pdf-biweekly" style="margin-left: 8px;">Bi-Weekly Report</label>';
        html += '</div>';
        
        html += '<div class="pdf-option" style="margin: 10px 0;">';
        html += '<input type="radio" id="pdf-monthly" name="pdf-type" value="monthly">';
        html += '<label for="pdf-monthly" style="margin-left: 8px;">Monthly Report</label>';
        html += '</div>';
        
        html += '<div class="pdf-option" style="margin: 10px 0;">';
        html += '<input type="radio" id="pdf-subject" name="pdf-type" value="subject">';
        html += '<label for="pdf-subject" style="margin-left: 8px;">Subject Report</label>';
        html += '</div>';
        
        // Subject selector (hidden by default)
    ...
    generatePDF() {
        ...
        switch(selectedType) {
            ...
            case 'subject':
                ...
                report = this.dataManager.generateSubjectReport(subject);
    ...
    exportAsPDF(content, filename) {
        // Using jsPDF library - make sure it's included in your HTML
        ...
        document.getElementById('report-content').innerHTML = 
            `<div class="alert alert-success">PDF "${filename}.pdf" has been generated and downloaded.</div>`;
    }
    ...
    showEmailForm() {
        ...
        html += '<div class="form-group" style="margin-bottom: 15px;" id="email-subject-container">';
        html += '<label>Subject (for subject report):</label>';
        ...
        // Show/hide subject input
        document.getElementById('email-report-type').addEventListener('change', (e) => {
            const subjectContainer = document.getElementById('email-subject-container');
    ...
    sendEmail() {
        ...
        if (reportType === 'subject' && !customSubject) {
        ...
        switch(reportType) {
            ...
            case 'monthly':
                ...
                break;
            case 'subject':
                report = this.dataManager.generateSubjectReport(customSubject);
                break;
        }
        
        // In a real app, you would send this to a backend server
        // For now, we'll create a mailto link
        const subject = `WorkLogPro ${reportType.charAt(0).toUpperCase() + reportType.slice(1)} Report`;
        ...
        document.getElementById('report-content').innerHTML = 
            `<div class="alert alert-success">Email client opened. Report is ready to send to ${toEmail}.</div>`;
    }
    ...
    showClaimFormOptions() {
        ...
        document.getElementById('generate-claim').addEventListener('click', () => {
            this.generateClaimForm();
        });
        
        document.getElementById('download-claim-pdf').addEventListener('click', () => {
            this.downloadClaimAsPDF();
        });
    }
    ...
    generateClaimForm() {
        ...
        const claim = this.dataManager.generateClaimForm(claimType, endDate);
    ...
    showReportContent(report, title) {
        ...
        html += '<button id="print-report" class="btn btn-sm btn-outline-secondary">Print</button>';
        html += '</div>';
        html += `<pre class="report-content" style="background: #f8f9fa; padding: 20px; border-radius: 5px; max-height: 500px; overflow-y: auto;">${report}</pre>`;
        html += '</div>';

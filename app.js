...
async function populateStudentDropdowns() {
    ...
    try {
        ...
        const studentDropdowns = [
            ...
            document.querySelector('select[name="student"]'), // Any other student selects
        ...
        // Also populate attendance checkboxes if needed
...
function populateAttendanceStudents(students) {
    ...
    // Clear existing checkboxes except the first one if it exists
...
// Call this function when forms load or when students are updated
function setupFormStudentPopulation() {
    ...
    // Also populate when switching to relevant tabs
...
// Update your existing form handlers to include student population
function setupFormHandlers() {
    // Student form
    const studentForm = document.getElementById('studentForm');
    if (studentForm) {
...
// Update your student submit handler to refresh dropdowns after adding a student
async function handleStudentSubmit(e) {
    e.preventDefault();
    const user = auth.currentUser;
    if (!user) return;
...
// Update your main initialization to populate students on app load
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Wait for auth and then populate student dropdowns
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            try {
                ...
                // POPULATE STUDENT DROPDOWNS AFTER DATA LOAD
                populateStudentDropdowns();
